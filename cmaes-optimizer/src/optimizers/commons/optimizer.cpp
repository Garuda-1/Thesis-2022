...
common::solver_output optimizer::evaluate_and_record(common::sample& sample, const common::optimizer_options& options) {
...
common::solver_output optimizer::evaluate_and_record(
  ...
  common::log();
  common::log(
      std::to_string(sample_fitness) + " | " + std::to_string(static_cast<uint64_t>(best_fitness)) + " | " +
