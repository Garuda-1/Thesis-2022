...
common::cnf_er gaer_optimizer::mutate(
    const common::cnf_er& cnf, const std::function<double(void)>& rnd, double shrink_scale) {
...
void gaer_optimizer::report_generation(
...
common::cnf_er gaer_optimizer::crossover(
    const common::cnf_er& a, const common::cnf_er& b, const std::function<double(void)>& rnd) {
...
ssize_t gaer_optimizer::fit() {
  ...
  ga.mutate =
      std::bind(&gaer_optimizer::mutate, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
  ga.crossover =
